...
final class RouteV2SessionFilter extends OncePerRequestFilter {
		...
		switch (session.status()) {
			...
			case VALID -> {
				if (!"route:v2:itx".equals(session.scope())) {
					metrics.recordResponse(422, "ROUTE_SCOPE_INVALID");
					RouteV2ErrorWriter.write(response, 422, "ROUTE_SCOPE_INVALID", "지원하지 않는 경로예요");
					return;
				}
	...
	private void unauthorized(HttpServletResponse response) throws IOException {
		metrics.recordResponse(401, "ROUTE_SESSION_REQUIRED");
		RouteV2ErrorWriter.write(response, 401, "ROUTE_SESSION_REQUIRED", "다시 시도");
	}
	...
	private void rateLimited(HttpServletResponse response, Instant now, Instant expiresAt) throws IOException {
		...
		RouteV2ErrorWriter.write(response, 429, "ROUTE_RATE_LIMITED", "잠시 후 다시 시도");
	}
}
