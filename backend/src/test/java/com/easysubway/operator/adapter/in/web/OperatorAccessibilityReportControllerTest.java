...
class OperatorAccessibilityReportControllerTest {
	...
	void operatorGetsAccessibilityReport() throws Exception {
		mockMvc.perform(get("/operator/api/accessibility-report")
			...
			.andExpect(jsonPath("$.data.stationQualityRows[0].description").value("기본 정보만 있음"))
			.andExpect(jsonPath("$.data.stationQualityRows[1].description").value("시설 정보 확인됨"))
			.andExpect(jsonPath("$.data.stationQualityRows[2].description").value("쉬운 길 안내 가능"))
			.andExpect(jsonPath("$.data.stationQualityRows[3].description").value("고장·공사 반영됨"))
