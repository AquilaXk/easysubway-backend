...
class RouteSearchControllerTest {
	...
	void postRouteSearchCreatesRouteAndGetReturnsStoredResult() throws Exception {
		var result = mockMvc.perform(post("/api/v1/routes/search")
			...
			.andExpect(jsonPath("$.data.recommendationReasons[0]").value("엘리베이터 동선을 우선했어요"))
			.andExpect(jsonPath("$.data.recommendationReasons[1]").value("계단 없는 출구를 확인했어요"))
			.andExpect(jsonPath("$.data.recommendationReasons[2]").value("유모차 이동에 맞춰 넓은 동선을 확인했어요"))
