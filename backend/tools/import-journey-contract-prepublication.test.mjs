...
test("importer는 prepublication descriptor와 manifest의 created annotation exact binding만 수락한다", () => {
  mkdirSync(join(root, "backend/build"), { recursive: true }); const directory = mkdtempSync(join(root, "backend/build/journey-prepublish-annotation-test-"));
  try {
    const cases = [
      { descriptorMutate: (value) => { delete value.annotations; } },
      { descriptorMutate: (value) => { value.annotations.extra = "rejected"; } },
      { manifestMutate: (value) => { delete value.annotations; } },
      { manifestMutate: (value) => { value.annotations.extra = "rejected"; } },
      { manifestMutate: (value) => { value.annotations["org.opencontainers.image.created"] = "2026-08-16T12:34:56+09:00"; } },
      { descriptorMutate: (value) => { value.annotations["org.opencontainers.image.created"] = "2026-08-16T12:34:57Z"; } },
    ];
    for (const [index, options] of cases.entries()) {
      const caseDirectory = join(directory, `${index}`); mkdirSync(caseDirectory); const artifact = buildArtifact(caseDirectory, options), caller = callerMetadata(directory, artifact), lock = join(directory, `${index}.lock.json`), trust = join(directory, `${index}.trust.json`);
      assert.throws(() => run(artifact.zip, caller, lock, trust), /descriptor|manifest is invalid/);
      assert.equal(existsSync(lock), false); assert.equal(existsSync(trust), false);
    }
  } finally { rmSync(directory, { recursive: true, force: true }); }
});
...
test("importer는 ZIP을 읽기 전에 8 MiB 크기를 거부하고 출력을 보존한다", () => {
  mkdirSync(join(root, "backend/build"), { recursive: true }); const directory = mkdtempSync(join(root, "backend/build/journey-prepublish-zip-size-test-"));
  try {
    ...
    truncateSync(artifact.zip, 8 * 1024 * 1024 + 1); writeFileSync(caller, JSON.stringify({ ...JSON.parse(readFileSync(caller)), archiveSha256: sha(readFileSync(artifact.zip)) })); writeFileSync(lock, "old-lock\n"); writeFileSync(trust, "old-trust\n");
    assert.throws(() => run(artifact.zip, caller, lock, trust), /too large/i);
    assert.equal(readFileSync(lock, "utf8"), "old-lock\n"); assert.equal(readFileSync(trust, "utf8"), "old-trust\n");
  } finally { rmSync(directory, { recursive: true, force: true }); }
});

test("importer는 artifact와 caller 입력에 alias된 출력을 mutation 전에 거부한다", () => {
  mkdirSync(join(root, "backend/build"), { recursive: true }); const directory = mkdtempSync(join(root, "backend/build/journey-prepublish-alias-test-"));
  try {
    const artifact = buildArtifact(directory), caller = callerMetadata(directory, artifact), trust = join(directory, "trust.json"), lock = join(directory, "lock.json"); writeFileSync(lock, "old-lock\n"); writeFileSync(trust, "old-trust\n");
    for (const output of [artifact.zip, caller]) {
      const artifactBefore = readFileSync(artifact.zip), callerBefore = readFileSync(caller);
      assert.throws(() => run(artifact.zip, caller, output, trust), /input/i);
      assert.deepEqual(readFileSync(artifact.zip), artifactBefore); assert.deepEqual(readFileSync(caller), callerBefore); assert.equal(readFileSync(trust, "utf8"), "old-trust\n");
    }
  } finally { rmSync(directory, { recursive: true, force: true }); }
});

test("importer pair publish는 second rename failure에서 0644 bytes와 mode를 복원하고 stage를 남기지 않는다", () => {
  ...
  try {
    const artifact = buildArtifact(directory), caller = callerMetadata(directory, artifact), lock = join(directory, "lock.json"), trust = join(directory, "trust.json");
    writeFileSync(lock, "old-lock\n", { mode: 0o644 }); writeFileSync(trust, "old-trust\n", { mode: 0o644 }); chmodSync(lock, 0o644); chmodSync(trust, 0o644);
    ...
    assert.equal(readFileSync(lock, "utf8"), "old-lock\n"); assert.equal(readFileSync(trust, "utf8"), "old-trust\n");
    assert.equal(statSync(lock).mode & 0o777, 0o644); assert.equal(statSync(trust).mode & 0o777, 0o644); assert.deepEqual(requireNoStages(directory), []);
...
test("importer는 두 번째 stage write 실패에도 생성된 stage와 출력을 정리한다", () => {
  mkdirSync(join(root, "backend/build"), { recursive: true }); const directory = mkdtempSync(join(root, "backend/build/journey-prepublish-stage-test-"));
  try {
    const artifact = buildArtifact(directory), caller = callerMetadata(directory, artifact), lock = join(directory, "lock.json"), trust = join(directory, "trust.json"); writeFileSync(lock, "old-lock\n"); writeFileSync(trust, "old-trust\n");
    let writes = 0;
    assert.throws(() => importJourneyContractPrepublication(argumentsFor(artifact.zip, caller, lock, trust), { afterStageWrite: () => { writes += 1; if (writes === 2) throw new Error("second stage failed"); } }), /second stage failed/);
    assert.equal(readFileSync(lock, "utf8"), "old-lock\n"); assert.equal(readFileSync(trust, "utf8"), "old-trust\n"); assert.deepEqual(requireNoStages(directory), []);
  } finally { rmSync(directory, { recursive: true, force: true }); }
});
...
function requireNoStages(directory) { return readdirSync(directory).filter((name) => name.includes(".stage-")); }
