...
아래는 저장소의 API 계약 파일입니다. 세 계약 모두 요청 parameter·요청 본문·필수 헤더·응답 스키마·오류 응답을 담고 있고, 계약과 실제 API가 어긋나면 CI가 실패합니다. 경로 탐색 계약은 아직 보완 중(#38)이라 이 목록에 없으니 협의로 확인해 주세요. 서버 주소는 계약 파일에 적지 않습니다.
