...
test('tracked tests and policy are self-contained reviewed inventory evidence', () => {
  ...
  assert.equal(digest(readFileSync(new URL('../../backend/quality/spotbugs-suppression-policy.json', import.meta.url))), 'c2dcb2cfceca4f610de6ef758bd63028f468cd5396a2c6c73a94e163433fcad6');
...
test('tracked remediation policy preserves prior children and projects the current immutable ledger', () => {
  ...
  assert.deepEqual(lifecycle, { ledgerTotal: 195, reported: 11, fixRequired: 11, fixed: 111, falsePositiveExactSuppression: 71, acceptedBoundedRisk: 2, generatedOrNonOwnedExclusion: 0, unclassified: 0, missing: 0, duplicate: 0, stale: 0 });
...
test('Backend #110 datapack projection is exact', () => {
  ...
  assert.equal((readFileSync(new URL('../../backend/quality/spotbugs-exclude.xml', import.meta.url), 'utf8').match(/<Match>/g) ?? []).length, 74);
});
...
test('terminal suppression filters require one ordered exact method Match', () => {
  ...
  assert.deepEqual(reconcileLedger(terminal, terminal.findings.filter(({ disposition }) => disposition === 'FIX_REQUIRED')), { ledgerTotal: 195, reported: 11, fixRequired: 11, fixed: 111, falsePositiveExactSuppression: 71, acceptedBoundedRisk: 2, generatedOrNonOwnedExclusion: 0, unclassified: 0, missing: 0, duplicate: 0, stale: 0 });
  for (const mutate of [
