...
const policy = () => ({
  ...
  toolchain: {
    gradleVersion: null,
    spotbugsGradlePlugin: { id: 'com.github.spotbugs', requestedVersion: '6.2.2', buildScriptSha256: 'd19ed88d6c88fea39179aadde7c5b3b93da7222d4dc3024343ef59a1d8d2bbb1', implementationClass: null, implementationJarSha256: null },
...
test('tracked tests and policy are self-contained reviewed inventory evidence', () => {
  ...
  assert.equal(digest(readFileSync(new URL('../../backend/quality/spotbugs-suppression-policy.json', import.meta.url))), '0e09f53c317306111cf3b26980ab3b82e0f7066118cfecacd40dbe03664f72ce');
...
test('tracked enforced policy preserves prior children and projects the current immutable ledger', () => {
  ...
  assert.equal(gate.defaultPolicy, 'zero-finding-required-enforcement');
  assert.equal(gate.enforcementStatus, 'spotbugs-zero-finding-required-enforced');
  assert.equal(spotbugs.enforcement, 'required_fail_closed_zero_finding');
  assert.match(spotbugs.evidence.failMode, /spotbugsMain ignoreFailures=false/);
  assert.match(spotbugs.evidence.failMode, /spotbugsTest remains non-failing under NOT_REQUIRED_CURRENT/);
  const validationIndex = 0;
  for (const [label, mutate] of [
    ['Gradle version', (value) => { value.toolchain.gradleVersion = null; }],
    ['plugin class', (value) => { value.toolchain.spotbugsGradlePlugin.implementationClass = 'wrong.Plugin'; }],
    ['engine version', (value) => { value.toolchain.spotbugsEngine.toolVersion = '4.9.4'; }],
    ['engine classpath order', (value) => { value.toolchain.spotbugsEngine.classpath.reverse(); }],
    ['missing finding', (value) => { value.findings.pop(); }],
    ['duplicate finding', (value) => { value.findings.push(structuredClone(value.findings[0])); }],
    ['source digest', (value) => { value.findings[validationIndex].sourceSha256 = '0'.repeat(64); }],
    ['owner URL', (value) => { value.findings[validationIndex].ownerIssueUrl = 'https://example.invalid/owner'; }],
    ['owner state', (value) => { value.findings[validationIndex].ownerIssueState = 'CLOSED'; }],
    ['expiry', (value) => { value.findings[validationIndex].expiresAt = '2026-01-01'; }],
    ['source path', (value) => { value.findings[validationIndex].sourcePath = 'backend/src/main/java/**/*.java'; }],
  ]) { const invalid = structuredClone(tracked); mutate(invalid); assert.throws(() => validatePolicy(invalid, { today: '2026-08-10' }), label); }
});
...
test('Backend #237 JDBC route search repository remediation is source-complete', () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      '[Build][Backend][P1] JdbcRouteSearchRepository SpotBugs remediation',
      'OPEN',
      'Backend #237 reviewed this exact aggregate query as guaranteed to return one row whose current mapper always returns a non-null summary.',
      'Remove this suppression if the aggregate SQL or mapper no longer guarantees one non-null summary row, or the exact finding disappears.',
      'Review this decision when the aggregate SQL, row mapper, JdbcTemplate contract, analyzer, or Backend #237 owner-state changes.',
      '2026-11-13',
      {
        kind: 'EXCLUDE_FILTER_EXACT_METHOD',
        bugPattern: 'NP_NULL_ON_SOME_PATH_FROM_RETURN_VALUE',
        className: 'com.easysubway.route.adapter.out.persistence.JdbcRouteSearchRepository',
        methodName: 'summarizeRouteFeedbacks',
        params: '',
        returns: 'com.easysubway.route.domain.RouteFeedbackDashboardSummary',
        reason: 'Backend #237 exact guaranteed-one-row route feedback aggregate boundary.',
      },
    ],
  );
  const excludeFilter = readFileSync(new URL('../../backend/quality/spotbugs-exclude.xml', import.meta.url), 'utf8');
...
test('Backend #4 final enforcement is atomic and fail closed', () => {
  ...
  assert.doesNotMatch(build, /spotbugs \{\n\tignoreFailures =/);
  assert.match(build, /tasks\.named\('spotbugsMain', com\.github\.spotbugs\.snom\.SpotBugsTask\) \{\n\tignoreFailures = false\n/);
  assert.match(build, /tasks\.named\('spotbugsTest', com\.github\.spotbugs\.snom\.SpotBugsTask\) \{\n\tignoreFailures = true\n\}/);
...
test('terminal dispositions require child-owned non-generic review metadata', () => {
  ...
  const fixed = terminal.findings.find(({ disposition }) => disposition === 'FIXED');
  assert.ok(fixed);
  fixed.ownerIssueUrl = 'https://github.com/AquilaXk/easysubway-backend/issues/4';
  fixed.ownerIssueTitle = '[Build][Backend][P1] current SpotBugs findings 정리·enforcement 전환';
  fixed.reason = 'Exact current SpotBugs finding requires source-level review and remediation in Backend #4.';
  fixed.removalCondition = 'Remove this entry when the finding disappears or Backend #4 records a root-approved exact terminal disposition.';
  fixed.reviewTrigger = 'Review on any finding identity, source byte, analyzer toolchain, classpath, report schema, or owner-state change.';
