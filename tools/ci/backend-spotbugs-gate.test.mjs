...
test('tracked tests and policy are self-contained reviewed inventory evidence', () => {
  ...
  assert.equal(digest(readFileSync(new URL('../../backend/quality/spotbugs-suppression-policy.json', import.meta.url))), '9303911e378b1670e2e4995e698bbb43dae492d90aec13dd06471978ea9235d5');
...
test('tracked remediation policy preserves prior children and projects the current immutable ledger', () => {
  ...
  assert.deepEqual(lifecycle, { ledgerTotal: 195, reported: 18, fixRequired: 18, fixed: 109, falsePositiveExactSuppression: 66, acceptedBoundedRisk: 2, generatedOrNonOwnedExclusion: 0, unclassified: 0, missing: 0, duplicate: 0, stale: 0 });
...
test('terminal suppression filters require one ordered exact method Match', () => {
  ...
  assert.deepEqual(reconcileLedger(terminal, terminal.findings.filter(({ disposition }) => disposition === 'FIX_REQUIRED')), { ledgerTotal: 195, reported: 18, fixRequired: 18, fixed: 109, falsePositiveExactSuppression: 66, acceptedBoundedRisk: 2, generatedOrNonOwnedExclusion: 0, unclassified: 0, missing: 0, duplicate: 0, stale: 0 });
  for (const mutate of [
